...
def main():
	...
	for n in nodes:
		cpy = nodes.copy()
		cpy.remove(n)
		minimum = min(minimum, tsp(n, nodes, distances, memo))
	print(f'The min distance is {minimum}')
	return
...
def tsp(i:int, nodes:list, dist, memo:dict) -> int:
	...
	for node in nodes:
		copy = nodes.copy()
		copy.remove(node)
		minimum = min(minimum, tsp(node, copy, dist, memo) + dist[node][i])

	# save best for this subset
	memo[i][toString(nodes)] = minimum
	return minimum
