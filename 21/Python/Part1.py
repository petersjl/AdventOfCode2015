...
def main():
	...
	lowest = findLowestCostLoadout(loadouts)
	print(f'The lowest cost loadout for a win is:')
	print(lowest)
	return

def findLowestCostLoadout(loadouts:list[Loadout]) -> Loadout:
	if len(loadouts) == 0: return None
	lowest:Loadout = loadouts[0]
	for l in loadouts:
		if l.cost < lowest.cost: lowest = l
	return lowest
